...
class CorrGraph(nn.Module):
    ...
    def __init__(self, length=12, dim=1):
        ...
        # 生成的score分数。if 长度为1，则输出为batch,N,N特征矩阵; 在图卷积中不区分时间步。
        #               if 长度为12，则输出为batch,timesteps,N,N,在图卷积中区分时间步。
...
class Conv_withSpatialScore(nn.Module):  # in the sencond blocks;which focus on the change
    ...
    # 输出形状        B,T,N,F
    #
    def __init__(self, num_of_features, out_channels, device):  # K,cheb,输入通道数，输出维度。节点个数，gpu
...
class TCN(nn.Module):
    def __init__(self, length, model_dim):
        ...
        self.Gelu = nn.GELU()
        self.TempConv2 = nn.Conv2d(model_dim * 2, model_dim, kernel_size=(1, 3), stride=(1, 1))
        self.normalize1 = nn.LayerNorm(model_dim)

        self.relu = nn.ReLU()
        self.TempConv3 = nn.Conv2d(model_dim, model_dim, kernel_size=(1, 4), stride=(1, 1))
        self.normalize2 = nn.LayerNorm(model_dim)

        self.Final_Conv = nn.Conv2d(self.t_length * 2, self.t_length, stride=1, kernel_size=(1, 1))
